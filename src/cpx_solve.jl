function optimize!(model::Model)
  ...
  stat = if model.has_int
    @cpx_ccall_intercept(mipopt, Cint, (Ptr{Void}, Ptr{Void}), model.env.ptr, model.lp)
  elseif model.has_qc
    @cpx_ccall_intercept(qpopt, Cint, (Ptr{Void}, Ptr{Void}), model.env.ptr, model.lp)
  else
    @cpx_ccall_intercept(lpopt, Cint, (Ptr{Void}, Ptr{Void}), model.env.ptr, model.lp)
  end
  if stat != 0

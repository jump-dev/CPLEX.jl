...
mutable struct Optimizer <: MOI.AbstractOptimizer
    ...
    inner::Model
    # The CPLEX environment. If `nothing`, a new environment will be created
    # on `MOI.empty!`.
...
function _indices_and_coefficients(
    ...
    for (i, term) in enumerate(f.quadratic_terms)
        ...
        # CPLEX does the same according to page 14 of the reference Manual.
...
#=
From what I can tell there is no constant term in CPLEX model
...
#=
Semicontinuous and Semiinteger not originally supported in CPLEX. Can be added later
